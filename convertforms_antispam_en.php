...
if (count($compareFieldsNorm) === 2) {
    ...
    if (isset($post[$keys[0]], $post[$keys[1]]) && trim($post[$keys[0]]) === trim($post[$keys[1]]) && trim($post[$keys[0]]) !== '') {
        writeSpamLogEntry('Die Felder dürfen nicht denselben Inhalt haben. Please check your entries. IP: ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown') . ' | fields: ' . json_encode([
...
foreach ($randomCheckFields as $field => $label) {
    ...
    if ($trigger) {
        writeSpamLogEntry('Spamverdacht: Ihre Eingabe in „' . $label . '“ ist verdächtig. Please check your entries. IP: ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown') . ' | fields: ' . json_encode([
            'field' => $field,
...
foreach ($post as $field => $value) {
    if (detectSqlInjection($value)) {
        writeSpamLogEntry('Verdacht auf schädliche Eingabe in Feld „' . $field . '“. Please check your entries. IP: ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown') . ' | fields: ' . json_encode([
            $field => $value
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), $logFilePath);
...
if ($debugMode) {
    writeSpamLogEntry('Debug mode: Keine Probleme erkannt – Formular wurde nicht gesendet (Simulation). IP: ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown') . ' | fields: ' . json_encode([], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), $logFilePath);
